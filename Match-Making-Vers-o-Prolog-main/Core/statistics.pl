...
% --- CLASSIFICAÇÃO ---
% (O código de classificação está perfeito, não mexi em nada)
...
calcular_distancias(Turno, Cidades, Distancias) :-
    ...
    % AJUSTE 1: O maplist vai preencher os dois últimos argumentos de distancia_por_time
...
% AJUSTE 2: Agora aceita 4 argumentos para ser compatível com o maplist acima
distancia_por_time(Turno, Cidades, Time, dist_time(Time, TotalKM)) :-
